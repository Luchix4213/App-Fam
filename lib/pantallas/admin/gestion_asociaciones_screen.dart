...
class _GestionAsociacionesScreenState extends State<GestionAsociacionesScreen> {
  ...
  void _filter() {
    ...
    setState(() {
      _filteredAsociaciones = _asociaciones.where((a) {
        ...
        final municipio = (a['municipio'] ?? '').toLowerCase();
  ...
  Future<void> _loadData() async {
    ...
    try {
      final futures = await Future.wait([
        ApiService.getAllAsociaciones(estado: _filterState),
        ApiService.getDepartamentos(isAdmin: true),
      ]);
      final asocRes = futures[0];
      final depRes = futures[1];
      ...
      if (depRes['success'] == true) {
        setState(() {
          _departamentos = depRes['data'];
        });
      }
    } catch (e) {
                 ...
                 child: Container(
                   ...
                   child: TextField(
                     ...
                     decoration: InputDecoration(
                       hintText: "Buscar por nombre, alias o municipio...",
  ...
  Widget _buildAsociacionCard(Map<String, dynamic> a) {
    final tipo = (a['tipo'] ?? 'AMDES').toString().toUpperCase();
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    if (a['presidente'] != null && a['presidente'].toString().isNotEmpty)
                      Text(
                        'Presidente: ${a['presidente']}',
                        style: const TextStyle(fontSize: 12, color: Color(0xFF546E7A)),
                      ),
                    if (a['municipio'] != null && a['municipio'].toString().isNotEmpty)
                       Text(
                    ...
                    Wrap(
                      ...
                      runSpacing: 4,
                      children: [
                        _buildBadge(tipo, Colors.blue.withOpacity(0.1), Colors.blue.shade700),
                        _buildBadge(
                          estado, 
...
class AsociacionForm extends StatefulWidget {
  ...
  const AsociacionForm({super.key, this.asociacion, required this.departamentos, required this.onSave});
...
class _AsociacionFormState extends State<AsociacionForm> {
  ...
  final _telPrivadoCtrl = TextEditingController();
  final _faxCtrl = TextEditingController();
  final _emailPublicoCtrl = TextEditingController();
  final _emailPrivadoCtrl = TextEditingController();
  ...
  void initState() {
    ...
    if (widget.asociacion != null) {
      ...
      _presidenteCtrl.text = a['president'] ?? a['presidente'] ?? ''; // Typo check
      _municipioCtrl.text = a['municipio'] ?? '';
      _telPublicoCtrl.text = a['telefono_publico'] ?? '';
      _telPrivadoCtrl.text = a['telefono_personal'] ?? '';
      _faxCtrl.text = a['telefono_fax'] ?? '';
      _emailPublicoCtrl.text = a['correo_publico'] ?? '';
      _emailPrivadoCtrl.text = a['correo_personal'] ?? '';
      _direccionCtrl.text = a['direccion'] ?? '';
      _tipo = (a['tipo'] ?? 'AMDES').toString().toUpperCase();
      _estado = (a['estado'] ?? 'ACTIVO').toString().toUpperCase();
      _selectedDeptoId = a['id_departamento'];
    }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      final data = {
        ...
        'presidente': _presidenteCtrl.text,
        'municipio': _municipioCtrl.text,
        'telefono_publico': _telPublicoCtrl.text,
        'telefono_personal': _telPrivadoCtrl.text, 
        'telefono_fax': _faxCtrl.text,
        'correo_publico': _emailPublicoCtrl.text,
        'correo_personal': _emailPrivadoCtrl.text,
        'direccion': _direccionCtrl.text,
        'tipo': _tipo,
        'estado': _estado.toLowerCase(),
        if (_selectedDeptoId != null) 'id_departamento': _selectedDeptoId.toString(),
      };
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    Expanded(child: _buildTextField(_aliasCtrl, "Alias")),
                    const SizedBox(width: 12),
                    Expanded(child: _buildTextField(_municipioCtrl, "Municipio")),
                  ]),
                  const SizedBox(height: 12),
                  _buildTextField(_presidenteCtrl, "Presidente"),
                  ...
                  Row(children: [
                    ...
                    const SizedBox(width: 12),
                    Expanded(child: _buildTextField(_telPrivadoCtrl, "Tel. Privado")),
                  ]),
                  const SizedBox(height: 12),
                  _buildTextField(_emailPublicoCtrl, "Correo Público"),
                  const SizedBox(height: 12),
                  _buildTextField(_emailPrivadoCtrl, "Correo Privado"),
                  const SizedBox(height: 12),
                  _buildTextField(_faxCtrl, "Fax"),
                  const SizedBox(height: 12),
                  ...
                  DropdownButtonFormField<String>(
                    value: _tipo,
                    items: ['AMDES', 'AMB', 'ACOBOL'].map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
                    onChanged: (v) => setState(() => _tipo = v!),
                    decoration: _inputDec("Tipo"),
                  ),
                  const SizedBox(height: 12),
                  DropdownButtonFormField<int>(
                    value: _selectedDeptoId,
                    items: widget.departamentos.map<DropdownMenuItem<int>>((d) => DropdownMenuItem(value: d['id'], child: Text(d['nombre']))).toList(),
                    onChanged: (v) => setState(() => _selectedDeptoId = v),
                    decoration: _inputDec("Departamento"),
                  ),
                  const SizedBox(height: 12),
                   DropdownButtonFormField<String>(
                    value: _estado,
                    items: ['ACTIVO', 'INACTIVO'].map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
                    onChanged: (v) => setState(() => _estado = v!),
                    decoration: _inputDec("Estado"),
                  ),
